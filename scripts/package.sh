...
VERSION="$(grep CFBundleShortVersionString -A0 "$ROOT/Voooxly.spec" | grep -o '[0-9.]*' | head -1)"
SHARE="$ROOT/dist/share"
ZIP="$ROOT/dist/Voooxly-v${VERSION}-share.zip"
...
rm -rf "$SHARE" "$ZIP"
mkdir -p "$SHARE"
ditto dist/Voooxly.app "$SHARE/Voooxly.app"
xattr -cr "$SHARE/Voooxly.app"
...
VOOOXLY — private, on-device voice dictation for macOS (Apple Silicon)
...
Install:
  1. Drag Voooxly.app into /Applications and open it.
     (If macOS refuses to open it: run `bash install.sh` instead.)
  2. System Settings → Privacy & Security: add /Applications/Voooxly.app
...
AI polish (optional): Voooxly auto-detects whatever you have — Ollama
running locally, or an ANTHROPIC_API_KEY / OPENAI_API_KEY in
~/.voooxly/.env. With none of them it pastes the raw transcription
